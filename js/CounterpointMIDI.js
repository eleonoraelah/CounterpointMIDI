// Recording MIDI signals
	...
	// Creating the duration arrays for the First and Second Species Counterpoints
		...
		function ctpMaking(){
			...
			index = first_ctp2_index;
			for (ctp_i = first_cf_index; ctp_i<melody_array.length; ctp_i++){

				// First Species Ctp array
				if (ctp_i == 0){
					ctp1_array[ctp_i] = melody_array[0][1] + cons_first_notes[Math.floor(Math.random() * cons_first_notes.length)];
				} else {
					while ((!possible_notes.includes(ctp1_array[ctp_i])) || (Math.abs(ctp1_array[ctp_i]-ctp1_array[ctp_i-1])>7)){
				...
				// Second Species Ctp array	
					...
					while (!possible_notes.includes(ctp2_array[ctp2_i+1])){ // || (Math.abs(ctp2_array[ctp2_i]-ctp2_array[ctp2_i-1])>7)
					//dissonant notes
				...
				if (ctp2_i >= 2 && ctp2_i<ctp2_duration.length){
					if (index = 0){
						ctp2_array[0] = melody_array[0][1] + cons_first_notes[Math.floor(Math.random() * cons_first_notes.length)];
					} else {
						while((!possible_notes.includes(ctp2_array[index])) || (Math.abs(ctp2_array[index]-ctp2_array[index-1])>7)){
							ctp2_array[index] = cf_ctp2[index][1] + cons_notes[Math.floor(Math.random() * cons_notes.length)];
						}
						index++;
					}
				}
			...
			if (send == false){
				if ((melody_array[melody_array.length-1][1] - melody_array[melody_array.length-2][1] == 1) || (melody_array[melody_array.length-1][1] - melody_array[melody_array.length-2][1] == 2)){
					...
					ctp1_array[melody_array.length-2] = possible_notes[possible_notes.indexOf(ctp1_array[melody_array.length-1]) + 1];
					...
					ctp2_array[cf_ctp2.length-2] = possible_notes[possible_notes.indexOf(ctp2_array[cf_ctp2.length-1]) + 1];
