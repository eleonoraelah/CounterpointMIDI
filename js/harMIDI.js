...
*/
		...
		function startPlayingSecondCtp(){
			...
			if (y<ctp2_duration.length){
				...
				drawSecondCtp(midi_freq_second);
	...
	//CANVAS
		...
		function clearScreen(){
			...
			for (var i = 1; i<25; i++){
			  ...
			  canvas_context.lineWidth = 0.3;
			  canvas_context.moveTo(0,i*h);
			  canvas_context.lineTo(WIDTH,i*h);
		...
		function drawChords(mFreq,ctpFreq) {
			y_canvas = (mFreq - 48)*dy;
			var style = "#0095DD";
			y_1ctp = (ctpFreq - 48)*dy;
		...
		function drawSecondCtp(ctpFreq) {	
			/*y_canvas = (mFreq - 48)*dy;
			var style = "#0095DD";*/
			y_1ctp = (ctpFreq - 48)*dy;
			var style_ctp = "#FF8C00";
