// Recording MIDI signals
	...
	// Creating the duration arrays for the First and Second Species Counterpoints
		...
		function getChordDuration(){
			...
			ctp2_duration = ctp2_duration.fill(melody_duration/2,counter_second-1,ctp2_duration.length-1);
			...
			dx_2ctp = dx_2ctp.fill(dx/2,counter_second-1,ctp2_duration.length-1);
			...
			ctp2_array[counter_second-1] = melody_array[first_melody_index][1] + cons_first_notes[Math.floor(Math.random() * cons_first_notes.length)];
			ctp2_array[counter_second] = melody_array[first_melody_index][1] + diss_notes[Math.floor(Math.random() * diss_notes.length)];
			...
			//dissonant notes
				ctp2_array[ctp2_i+1] = melody_array[chord_i][1] + diss_notes[Math.floor(Math.random() * diss_notes.length)];
			//melody notes for 2ctp
				melody_second[ctp2_i] = melody_array[chord_i][1];
				melody_second[ctp2_i+1] = melody_array[chord_i][1];
			}
			ctp2_array[ctp2_array.length-2] =  melody_array[melody_array.length-1][1] + cons_notes[Math.floor(Math.random() * cons_notes.length)];
			/*ctp2_array.length = ctp2_duration.length;
			melody_second.length = ctp2_array.length;*/
