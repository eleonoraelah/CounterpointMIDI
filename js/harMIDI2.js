// Recording MIDI signals
	...
	// Creating the duration arrays for the First and Second Species Counterpoints
		...
		function getChordDuration(){
			...
			ctp2_array[first_melody_index] = melody_array[first_melody_index][1] + cons_first_notes[Math.floor(Math.random() * cons_first_notes.length)];
			ctp2_array[first_melody_index+1] = melody_array[first_melody_index][1] + diss_notes[Math.floor(Math.random() * diss_notes.length)];
			console.log("ctp2_array[0] = " + ctp2_array[first_melody_index]);
			console.log("ctp2_array[1] = " + ctp2_array[first_melody_index+1]);

			melody_second[first_melody_index] = melody_array[first_melody_index];
			melody_second[first_melody_index+1] = melody_array[first_melody_index];

			for (chord_i = first_melody_index+1; chord_i<melody_array.length; chord_i++){
			// First Species Ctp array
				ctp1_array[chord_i] = melody_array[chord_i][1] + cons_notes[Math.floor(Math.random() * cons_notes.length)];
			}

			//Clausola vera
			if (melody_array[melody_array.length-1][1] - melody_array[melody_array.length-2][1] == 1){
				...
				console.log("ctp1_array[length-2] = " + ctp1_array[melody_array.length-2]);
				console.log("ctp1_array[length-1] = " + ctp1_array[melody_array.length-1]);

			} else if (melody_array[melody_array.length-2][1] - melody_array[melody_array.length-1][1] == 2){
				ctp1_array[melody_array.length-1] = melody_array[melody_array.length-1][1] + cons_last_notes[Math.floor(Math.random() * cons_last_notes.length)];
				ctp1_array[melody_array.length-2] = ctp1_array[melody_array.length-1] - 1;
			}

			ctp2_i = first_melody_index+2;
			console.log("ctp2_i = " + ctp2_i);

			while (ctp2_i<ctp2_duration.length){
			// Second Species Ctp array	

			//consonant notes
				ctp2_array[ctp2_i] = melody_array[ctp2_i-1][1] + cons_notes[Math.floor(Math.random() * cons_notes.length)];
			//dissonant notes
				ctp2_array[ctp2_i+1] = melody_array[ctp2_i-1][1] + diss_notes[Math.floor(Math.random() * diss_notes.length)];
			//melody notes for 2ctp
				melody_second[ctp2_i] = melody_array[ctp2_i-1][1];
				melody_second[ctp2_i+1] = melody_array[ctp2_i-1][1];
				ctp2_i += 2;
				console.log("ctp2_i = " + ctp2_i);
			}

			ctp2_array[ctp2_array.length-2] =  melody_array[melody_array.length-1][1] + cons_last_notes[Math.floor(Math.random() * cons_last_notes.length)];
			console.log("ctp2_array[ctp2_array.length-2] = " + ctp2_array[ctp2_array.length-2]);
		}
